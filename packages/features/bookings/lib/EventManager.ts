        ...
        if (isLocationChanged || isBookingRequestedReschedule || isDailyVideoRoomExpired) {
          ...
          if (isDedicated) {
            ...
            if (updatedEvent) {
              ...
              booking.references = booking.references.map((ref) => {
                if (ref.type === result.type) {
                  return {
                    ...ref,
                    uid: updatedEvent.id ? String(updatedEvent.id) : ref.uid,
                    meetingId: updatedEvent.id ? String(updatedEvent.id) : ref.meetingId,
                    meetingPassword: updatedEvent.password ?? ref.meetingPassword,
                    meetingUrl: updatedEvent.url || ref.meetingUrl,
                  };
                }
                return ref;
              });
